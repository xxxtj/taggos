...
class SafeMySQL
{
	...
	public function getRow()
	{
		...
		if ($res = $this->rawQuery($query)) {
			...
			$this->free($res);
			if($ret){
				return @$this->array_to_object($ret);
			}else{
				return FALSE;
			}
	...
	private function prepareQuery($args)
	{
		...
		$array = preg_split('~(\?[nsiuapel])~u',$raw,null,PREG_SPLIT_DELIM_CAPTURE);
		$anum  = count($args);
	...
	private function escapeStringForLike($value)
	{
		if ($value === NULL)
		{
			return 'NULL';
	...
	private function createLIKE($data)
	{
		if (is_array($data))
		{
			$this->error("SET (?l) placeholder expects string, ".gettype($data)." given");
			return;
		}
		if (!$data)
		{
			$this->error("Empty string for SET (?l) placeholder");
			return;
		}
		 
			$query = 'LIKE '.$this->escapeStringForLike($data);
