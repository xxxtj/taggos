<?
Class Files{
    private static $path = "uploads/images/";
    
    public  function upload_file($file, $extensions, $filepath = null) {
        $ext    = ['png','jpeg','jpg','gif'];
        $path = (is_null($filepath)) ? self::$path : $filepath;
        $new_file_name = md5(_uniq());
        $destination = $path .'_full/'. $new_file_name . '.' . $this->file_extension($file);
        $file_name = $new_file_name . '.' . self::file_extension($file);

        if($this->check_file_extension(self::file_extension($file), $extensions )){
            if(in_array($this->file_extension($file), $ext)){
                $this->resize($file, $file_name, 100, 100, $path.'100_100/');
                $this->resize($file, $file_name, 250, 250, $path.'250_250/');
                $this->resize($file, $file_name, 380, 250, $path.'380_250/');
                $this->resize($file, $file_name, 600, 400, $path.'400_600/');
                $this->resize($file, $file_name, 500, 500, $path.'500_500/');
            }
    ...
    private function check_file_extension($file_extension, $extensions){ 
    ...
    private function resize($file, $new_file_name, $width, $height, $path){
